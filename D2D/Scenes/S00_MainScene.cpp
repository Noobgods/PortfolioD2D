...
{
	ImGui::LabelText("shake", "%f", fBubbleShake);
	ImGui::LabelText("distance", "%f", fBubbleDistance);
...
void S00_MainScene::CeilingDown()
{
	/// Bubbles 흔들기
	if (fStageTime >= 15) {
		for (UINT i = 0; i < BALL_LINE_SIZE_Y; i++) {
...
void S00_MainScene::BubbleStitch(UINT x_, UINT y_)
{
	Vector2 sPos = shootBubble->Position();
...
void S00_MainScene::BubbleExplodeCheck(UINT x, UINT y)
{
	// x, y가 범위 밖인 경우
...
void S00_MainScene::BubbleExplodeDelete()
{
	if (uiExplodeCount < 3) {
...
void S00_MainScene::BubbleAirCheck(UINT x, UINT y)
{
	// 이미 체크 되어있으면
...
void S00_MainScene::BubbleAirDelete()
{
	if (stageState == State::GAME) {
		for (UINT i = 0; i < BALL_LINE_SIZE_Y; i++) {
			for (UINT j = 0; j < BALL_LINE_SIZE_X; j++) {
...
void S00_MainScene::BubbleDelete()
{
...
void S00_MainScene::MonsterDelete()
{
